...
async function getDb() {
  if (!_client) {
    _client = createClient({
      url: process.env.TURSO_DATABASE_URL || 'file:./data/contractai.db',
...
async function initDb(client) {
  // For local file-based SQLite, ensure data directory exists
  const url = process.env.TURSO_DATABASE_URL || 'file:./data/contractai.db'
