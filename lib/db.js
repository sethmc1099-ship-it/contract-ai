...
async function getDb() {
  ...
  if (!_initialized) {
    // Prevent concurrent init calls (multiple requests during cold start)
    if (!_initPromise) {
      _initPromise = initDb(_client).then(() => { _initialized = true }).catch(err => {
        _initPromise = null
        throw err
      })
    }
...
async function initDb(client) {
  // Create tables one at a time (more reliable than executeMultiple with Turso)
  ...
  const variantCount = countResult.rows[0]
  if (Number(variantCount.cnt) === 0) {
    await client.execute({
