    ...
    const message = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: 'text', text: 'Transcribe all text from this contract photo/screenshot exactly as it appears, preserving structure. Return only the transcribed text, no commentary. If the image is blurry, dark, or unreadable, respond with exactly: UNREADABLE_IMAGE' },
        ],
      }],
    })
    const text = message.content[0].text.trim()
    if (text === 'UNREADABLE_IMAGE' || text.length < 50) {
...
module.exports = { analyzeContract, generateVariantCopy, extractTextFromImage }
