...
async function analyzeContract(documentText, documentName) {
  ...
  const userPrompt = `Please analyze this contract document and return a JSON response with this exact structure. Return ONLY valid JSON, no markdown, no code blocks, no preamble.
...
Required JSON structure:
{
  ...
  "risk_score": 75,
  "executive_summary": "2-3 sentence plain English summary of what this document is and the biggest concerns",
  "key_findings": [
    {
      "type": "risk | opportunity | missing | unusual",
      "severity": "low | medium | high | critical",
      "title": "Short title",
      "clause": "Exact quote from document (first 200 chars max)",
      "explanation": "Plain English explanation of what this means for the client",
      "recommendation": "Specific action the client should take or negotiate"
    }
  ],
  "missing_protections": ["list of standard protections not found in this document"],
  "negotiation_points": ["Top 3-5 things to negotiate before signing"],
  "should_sign": true,
  "lawyer_review_recommended": true,
  "lawyer_review_reason": "Specific reason why professional review is or is not recommended"
}`
