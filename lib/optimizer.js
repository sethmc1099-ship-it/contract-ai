...
async function getVariantForVisitor() {
  const variants = (await getAllVariants()).filter(v => v.active === 1)

  if (variants.length === 0) return 'variant_a'

  // If no data yet, pure random
  const totalVisits = variants.reduce((s, v) => s + Number(v.visits), 0)
  if (totalVisits < 10) {
  ...
  const samples = variants.map(v => ({
    id: v.id,
    sample: thompsonSample(Number(v.conversions), Number(v.visits)),
  }))
...
async function runOptimizer() {
  ...
  try {
    ...
    const variantMetrics = variants.map(v => {
      const visits = Number(v.visits)
      const conversions = Number(v.conversions)
      const revenue = Number(v.revenue)
      const convRate = visits > 0 ? conversions / visits : 0
      const revenuePerVisitor = visits > 0 ? revenue / visits : 0
      const status = visits < 30 ? 'learning' : 'sufficient_data'
      return { ...v, visits, conversions, revenue, convRate, revenuePerVisitor, status }
    })
    ...
    if (withData.length > 0) {
      ...
      if (winnerProb > 0.95) {
        ...
        await logOptimization(db, 'winner_identified', `${currentWinner} is the clear winner with ${(winnerProb * 100).toFixed(1)}% confidence`, { variant_id: currentWinner, win_probability: winnerProb })
      }
    }
    ...
    for (const v of variantMetrics) {
      if (v.status === 'sufficient_data' && v.convRate < 0.02 && v.visits > 50) {
        ...
        await db.execute({
          sql: `UPDATE ab_variants SET active=0, updated_at=unixepoch() WHERE id=?`,
          args: [v.id],
        })
        actions.push(`Disabled underperforming variant ${v.id} (${(v.convRate * 100).toFixed(1)}% conv rate after ${v.visits} visits)`)
        await logOptimization(db, 'variant_disabled', `Variant ${v.id} had <2% conversion after 50+ visits`, { variant_id: v.id, conv_rate: v.convRate })
      }
    }
      ...
      try {
        ...
        if (newCopy) {
          ...
          await db.execute({
            ...
            args: [newId, `Auto-Generated: ${newCopy.value_prop}`, JSON.stringify(newCopy)],
          })

          actions.push(`Generated new variant ${newId}: "${newCopy.headline}" at $${(newCopy.price_cents / 100).toFixed(2)}`)
          await logOptimization(db, 'variant_created', 'Auto-generated new variant due to underperformance', { variant_id: newId, config: newCopy })
        }
      } catch (err) {
...
async function logOptimization(db, action, reason, data) {
  try {
    await db.execute({
      sql: `INSERT INTO optimization_log (action, reason, data) VALUES (?, ?, ?)`,
      args: [action, reason, JSON.stringify(data)],
    })
  } catch (err) {
